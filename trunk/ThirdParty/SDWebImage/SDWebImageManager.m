...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didRecieveImageData:(NSNumber *)dataSize;
{
    [downloader retain];
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didRecieveImageData:)])
            {
                [delegate performSelector:@selector(webImageManager:didRecieveImageData:) withObject:self withObject:dataSize];
            }
        }
    }
